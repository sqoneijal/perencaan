...
const getColumns = ({ navigate, limit, offset }: ColumnDeps): Array<ColumnDef<Lists>> => [
   {
      ...
      cell: ({ row: { original } }) => {
         return (
            <>
               <Button variant="ghost" size="sm" onClick={() => navigate(`/pagu-anggaran/program-studi/actions/${getValue(original, "id")}`)}>
                  <Pencil />
               </Button>
               <ConfirmDialog
                  url={`/pagu-anggaran/program-studi/actions/${getValue(original, "id")}`}
                  refetchKey={["pagu-anggaran", "program-studi", limit, offset]}
               />
            </>
         );
      },
   ...
   {
      accessorKey: "sisa_pagu_fakultas",
      header: "sisa pagu fakultas",
      enableSorting: true,
      cell: ({ row: { original } }) => toRupiah(getValue(original, "sisa_pagu_fakultas")),
   },
];
