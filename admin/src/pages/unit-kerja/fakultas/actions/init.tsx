...
export function useSubmit() {
   ...
   const submit = usePostMutation<{ errors: Record<string, string> }>("/unit-kerja/fakultas/actions");
...
export function useButton() {
   ...
   useEffect(() => {
      setButton(
         <Button variant="outline" size="sm" onClick={() => navigate("/unit-kerja/fakultas")}>
            Batal
         </Button>
      );
      return () => {
...
export function useEditData() {
   ...
   const { data, isLoading, error } = useApiQuery<{
      ...
      queryKey: ["unit-kerja", "fakultas", "actions", id],
      url: `/unit-kerja/fakultas/actions/${id}`,
      options: { enabled: !!id },
   });

   if (error) {
      toast.error(error?.message);
      queryClient.removeQueries({ queryKey: ["unit-kerja", "fakultas", "actions", id] });
   }
