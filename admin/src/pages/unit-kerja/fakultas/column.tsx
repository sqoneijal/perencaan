...
const getColumns = ({ navigate, limit, offset }: ColumnDeps): Array<ColumnDef<Lists>> => [
   {
      ...
      cell: ({ row: { original } }) => {
         return (
            <>
               <Button variant="ghost" size="sm" onClick={() => navigate(`/unit-kerja/fakultas/actions/${getValue(original, "id")}`)}>
                  <Pencil />
               </Button>
               <ConfirmDialog
                  url={`/unit-kerja/fakultas/actions/${getValue(original, "id")}`}
                  refetchKey={["unit-kerja", "fakultas", limit, offset]}
               />
            </>
         );
      },
