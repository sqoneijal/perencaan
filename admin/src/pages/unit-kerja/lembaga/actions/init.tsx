...
export function useSubmit() {
   ...
   const submit = usePostMutation<{ errors: Record<string, string> }>("/unit-kerja/lembaga/actions");
         ...
         onSuccess: (data) => {
            ...
            if (data?.status) {
               queryClient.refetchQueries({
                  queryKey: ["unit-kerja", "lembaga", limit, offset],
               });
               toast.success(data.message);
               navigate("/unit-kerja/lembaga");
               return;
            }
...
export function useButton() {
   ...
   useEffect(() => {
      setButton(
         <Button variant="outline" size="sm" onClick={() => navigate("/unit-kerja/lembaga")}>
            Batal
         </Button>
      );
      return () => {
...
export function useEditData() {
   ...
   const { data, isLoading, error } = useApiQuery<{
      ...
      queryKey: ["unit-kerja", "lembaga", "actions", id],
      url: `/unit-kerja/lembaga/actions/${id}`,
      options: { enabled: !!id },
   });

   if (error) {
      toast.error(error?.message);
      queryClient.removeQueries({ queryKey: ["unit-kerja", "lembaga", "actions", id] });
   }
